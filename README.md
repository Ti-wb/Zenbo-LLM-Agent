...
本機設定採個人實體操作模式，在 Zenbo 上開啟設定即可修改；手機遙控配對不會取得設定權限。
...
- 在 Zenbo 點「啟用並顯示配對 QR」，用同一個 Wi-Fi 的手機相機掃描，
  即可直接配對並開啟相機與遙控網頁，手機不必輸入配對碼。QR 含有效 120 秒、僅能使用
  一次的配對碼；頁面讀取後立即從網址移除，不含 Hermes 金鑰。
  過期或換手機時，點「重新產生配對 QR」；這會停止動作並中斷舊控制權。
  無法掃碼時仍可手動開啟網址並使用備用配對碼。有多個區網位址時可在 QR 區選擇。
  遙控使用獨立的 Android `8788` HTTP 介面，閒置失聯會停止動作，
  不開放 Hermes 金鑰或設定。詳見[區網控制契約](contracts/local-runtime/remote-control.md)。
- Hermes 或本機 renderer 失聯時採保守策略，全域停止 App 擁有的移動／跟隨；
  不會在離線狀態持續執行原先的跟隨命令。

這版 App 要求八個裝置工具；須同步更新隨附的 `zenbo` 1.1.1 外掛並重新啟動既有 Hermes

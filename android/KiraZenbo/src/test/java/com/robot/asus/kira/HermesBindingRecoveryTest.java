...
public class HermesBindingRecoveryTest {
    @Test public void delayedOldSpeechFailureCannotDisconnectAReboundChannel() throws Exception {
        ...
        Host(OkHttpClient http) throws Exception {
            ...
            server.setDispatcher(new Dispatcher() {
                @Override public MockResponse dispatch(RecordedRequest request) {
                    ...
                    if (path.equals("/p/robot/api/sessions/api_existing")) return json("{\"session\":{\"id\":\"api_existing\"}}");
                    if (path.endsWith("/device-channel")) return new MockResponse().withWebSocketUpgrade(new WebSocketListener() {
                        @Override public void onMessage(WebSocket channel, String text) {
                            try {
                                ...
                                assertEquals("api_existing", message.getString("sessionId"));
                                socket.set(channel);
                                bindings.incrementAndGet();
                                if (acknowledgeBinding.get()) channel.send(BOUND);
